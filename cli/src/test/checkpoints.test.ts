...
import { AgentEmitter, AskUserManager, type AgentRuntime } from '../agent/runtime.js'
import { PermissionManager } from '../agent/permission.js'
import { TodoStore } from '../storage/todo.js'
import { stagePreState, markTouched, undoLastTurn, diffCheckpoints, relPath } from '../agent/checkpoints.js'
...
test('relPath: absolut & relatif dinormalisasi', () => {
  const cwd = os.tmpdir()
  assert.equal(relPath(cwd, 'a/b.txt'), 'a/b.txt')
  assert.ok(relPath(cwd, path.join(cwd, 'x/y.ts')).endsWith('x/y.ts'))
})

test('checkpoint: stage → mark → undo mengembalikan isi awal per turn', () => {
  ...
  let skipped = stagePreState(rt, 'cek.txt')
  ...
  const restored2 = undoLastTurn(rt)
...
test('checkpoint: undo tanpa history → kosong', () => {
  const rt = makeRuntime(os.tmpdir())
  assert.deepEqual(undoLastTurn(rt), [])
})
