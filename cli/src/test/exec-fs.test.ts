...
import { glob, grep } from '@topupsaja/core/agent/exec.js'
import { loadIgnore, isIgnored } from '@topupsaja/core/session/context.js'
...
test('glob: match pattern dan skip dir default + gitignore', async () => {
  const r1 = await glob({ pattern: '**/*.ts' })
  assert.ok(r1.ok)
  assert.ok(r1.output.includes('app.ts'))
  assert.ok(r1.output.includes('src/util.ts'))

  const r2 = await glob({ pattern: '**/*.js' })
  assert.ok(r2.ok)
  // dist/ di-exclude default — bundle.js tidak boleh muncul
  assert.ok(!r2.output.includes('bundle.js'))
})

test('grep: cari konten dengan nomor baris, skip ignored', async () => {
  const r1 = await grep({ pattern: 'export' })
  assert.ok(r1.ok)
  assert.ok(r1.output.includes('app.ts:1:'))
  assert.ok(r1.output.includes('src/util.ts:1:'))

  const r2 = await grep({ pattern: 'bundled' })
  assert.ok(r2.ok)
  assert.ok(!r2.output.includes('dist/bundle.js'))
})

test('ignore: semantik gitignore penuh via paket ignore', async () => {
  fs.writeFileSync(path.join(tmp, '.gitignore'), 'secret/\n*.log\n')
  const ig = await loadIgnore(tmp)
  ...
  const ig2 = await loadIgnore(path.join(tmp, 'src'))
