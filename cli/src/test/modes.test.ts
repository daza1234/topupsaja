...
import { buildSystemPrompt, isReadOnlyMode, resolveModeArg, ALL_MODES, MODE_ALIASES } from '../agent/modes.js'
import type { Mode } from '../agent/modes.js'
import { AgentSession, projectSessionDir, normalizeMode } from '../session/store.js'
...
test('MODE_ALIASES: plan→architect, act→code, identitas lainnya', () => {
...
test('resolveModeArg: case-insensitive, unknown → null', () => {
...
test('isReadOnlyMode: architect & ask read-only', () => {
...
test('modeSection: masing-masing mode punya seksi di system prompt', () => {
  const arch = buildSystemPrompt(tmp, 'architect')
  ...
  const ask = buildSystemPrompt(tmp, 'ask')
  ...
  const testMode = buildSystemPrompt(tmp, 'test')
...
test('load(): sesi lama tersimpan plan/act dimap ke architect/code', () => {
  ...
  assert.equal(AgentSession.load(tmp, s.id)!.mode, 'architect')

  raw.mode = 'act'
  fs.writeFileSync(file, JSON.stringify(raw))
  assert.equal(AgentSession.load(tmp, s.id)!.mode, 'code')

  // Mode baru tidak berubah.
  raw.mode = 'test' as unknown as Mode
  fs.writeFileSync(file, JSON.stringify(raw))
  assert.equal(AgentSession.load(tmp, s.id)!.mode, 'test')
})
